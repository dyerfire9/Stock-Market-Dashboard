...
const getSubStocks = async (req, res) => {
    ...
    const subStocks = await subStock.find({user_id}).sort({createAt: -1})
...
const addSubStock = async (req, res) => {
    ...
    if(!cts.valid(ticker)) {
        res.status(400).json({error: 'Please enter a valid ticker'})
    }
    try{
        const user_id = req.user._id
...
const delSubStock = async (req, res) => {
    ...
    const subStock = await subStock.findOneAndDelete({_id: id})
